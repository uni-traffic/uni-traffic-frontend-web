...
const ViolationEditModal = ({ isOpen, onClose, violation }: ViolationEditModalProps) => {
  const queryClient = useQueryClient();
  const [violationName, setViolationName] = useState(violation?.violationName ?? "0");
  ...
  useEffect(() => {
    ...
    setViolationName(violation?.violationName.toString() ?? "0");
  }, [violation]);
